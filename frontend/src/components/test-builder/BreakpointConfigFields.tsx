...
export function BreakpointConfigFields({ form }: ConfigFieldsProps) {
  const { register, formState: { errors } } = form;

  return (
    <>
      <Input label="Virtual Users (start)" type="number" error={errors.virtualUsers?.message} {...register('virtualUsers')} />
      <Input label="Step Size (users per step)" type="number" helperText="User increment for binary search" error={errors.stepSize?.message} {...register('stepSize')} />
